...
from utils import clean_folder, get_args
...
def main(host, user, args):
    ...
    if not args.run_local:
        ...
        if not exists:
            ...
            subprocess.run(
                ["ssh", host, "cd AuViMi;", host_python_path, "-m", "pip", "install", "-r", "requirements.txt"]
            )
        else:
    ...
    args_cli = [
        "--" + key + '="' + str(args[key]) + '"' for key in args if (args[key] is not None and str(args[key]) != "")
    ]
