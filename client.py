...
def draw_loss_graph(history, width, height):
    ...
    if len(history) < 2:
        cv2.putText(graph, "Waiting for data...", (10, height // 2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (100, 100, 100), 1)
        return graph
    ...
    cv2.putText(graph, f"Loss: {history[-1]:.2f}", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)

    # Draw Min/Max labels
    cv2.putText(graph, f"Max: {max(history):.1f}", (width - 80, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 200, 255), 1)
    cv2.putText(
        graph, f"Min: {min(history):.1f}", (width - 80, height - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 200, 255), 1
    )
